...
class EnvironmentReadinessV05Tests(unittest.TestCase):
    ...
    def test_environment_preflight_binds_interpreter_packages_smoke_and_capacity(self) -> None:
        ...
        probe = {
            "python": "3.10.11",
            "python_implementation": gate.TESTED_ENVIRONMENT["python_implementation"],
            "architecture": gate.TESTED_ENVIRONMENT["architecture"],
            "platform_system": gate.TESTED_ENVIRONMENT["platform_system"],
            "machine": gate.TESTED_ENVIRONMENT["machine"],
        ...
        with mock.patch.object(gate, "_sha256", return_value=gate.TESTED_ENVIRONMENT["executable_sha256"]), \
             ...
             mock.patch.object(gate, "required_historical_vector_replay", return_value={"status": "PASS", "required": True, "sample_count": 21, "classification": "ENVIRONMENT_PARITY_EVIDENCE / NOT_NEW_SCIENTIFIC_RESULT"}), \
        ...
        self.assertEqual(result["status"], "PASS")
        self.assertTrue(result["historical_vector_replay_required"])
    ...
    def test_environment_preflight_rejects_wrong_interpreter_or_package(self) -> None:
        ...
        probe = {
            "python": "3.10.11", "python_implementation": gate.TESTED_ENVIRONMENT["python_implementation"],
            "architecture": gate.TESTED_ENVIRONMENT["architecture"], "platform_system": gate.TESTED_ENVIRONMENT["platform_system"],
            "machine": gate.TESTED_ENVIRONMENT["machine"], "packages": {**gate.TESTED_ENVIRONMENT["required_packages"], "numpy": "0"},
            "distributions": dict(gate.TESTED_ENVIRONMENT["required_distributions"]),
            "project_imports": {"status": "PASS", "count": len(gate.project_local_import_closure(ROOT)), "modules": []}, "smoke": {},
        }
        with mock.patch.object(gate, "_sha256", return_value=gate.TESTED_ENVIRONMENT["executable_sha256"]), \
             mock.patch.object(gate, "validate_complete_model_directory", return_value={"status": "PASS_EXACT"}), \
             mock.patch.object(gate, "_interpreter_probe", return_value=probe):
