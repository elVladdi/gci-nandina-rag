...
class He4PhaseIPreGenerationTests(unittest.TestCase):
    ...
    def test_01_initial_head_is_ancestor_and_branch_is_authorized(self) -> None:
        self.assertEqual(
            subprocess.run(["git", "branch", "--show-current"], cwd=ROOT, check=True, text=True, stdout=subprocess.PIPE).stdout.strip(),
            "codex/exp04-rerun-v02",
        )
    ...
    def test_11_no_phase_j_or_k_output_exists_before_generation(self) -> None:
        self.assertFalse(any(path.name.startswith(("he4_validation", "he4_rubric", "gate_j", "gate_k")) for path in OUT.iterdir()))
