...
class EV04MRRContractV04Tests(unittest.TestCase):
    ...
    def test_ev04_aggregate_uses_canonical_control_and_has_zero_delta(self) -> None:
        canonical = runner.aggregate_baseline_metrics(
            "EV04",
            {},
            Path("."),
            {"expected_metrics": copy.deepcopy(self.metrics)},
        )
        rows = subject.produce_ev04_aggregate_comparison_v04(canonical, self.metrics)
        self.assertEqual(len(self.metrics), 92)
        self.assertEqual(len(self.metrics["metric_table"]), 27)
        self.assertEqual(len(rows), 28)
        self.assertEqual([row["metric"] for row in rows], list(subject.EV04_AGGREGATE_METRIC_ORDER))
        self.assertTrue(all(row["absolute_delta"] == 0.0 for row in rows))
        self.assertEqual(rows[2]["metric"], "mrr_101_200_contribution")
        self.assertEqual(rows[2]["absolute_delta"], 0.0)
        self.assertNotIn("mrr", [row["metric"] for row in rows])
        self.assertEqual(canonical["mrr_at_200"], canonical["mrr"])

    def test_contribution_mutation_is_visible_in_its_aggregate_row(self) -> None:
        corrected = copy.deepcopy(self.metrics)
        corrected["mrr_101_200_contribution_numerator"] += 1.0
        corrected["mrr_101_200_contribution"] += 1.0 / corrected["cases_evaluated"]
        rows = subject.produce_ev04_aggregate_comparison_v04(self.metrics, corrected)
        contribution = rows[2]
        self.assertNotEqual(contribution["original_numerator"], contribution["corrected_numerator"])
        self.assertNotEqual(contribution["absolute_delta"], 0.0)

    def test_contribution_contract_fail_closed_controls(self) -> None:
        mutations = {
            "missing contribution": lambda metrics: metrics.pop("mrr_101_200_contribution"),
            "missing numerator": lambda metrics: metrics.pop("mrr_101_200_contribution_numerator"),
            "denominator mismatch": lambda metrics: metrics.__setitem__("cases_evaluated", metrics["cases_evaluated"] + 1),
            "nonfinite contribution": lambda metrics: metrics.__setitem__("mrr_101_200_contribution", math.inf),
        }
        for label, mutate in mutations.items():
            with self.subTest(label=label):
                ...
                mutate(corrected)
                with self.assertRaises(subject.ContractViolation):
                    subject.produce_ev04_aggregate_comparison_v04(self.metrics, corrected)

    def test_unified_summary_accepts_28_row_ev04_aggregate(self) -> None:
        rows = subject.produce_ev04_aggregate_comparison_v04(self.metrics, self.metrics)
        ev04 = {"status": "PASS", "metrics": rows}
        summary = runner.common.build_unified_summary(
            {"status": "PASS", "metrics": []},
            ev04,
            {"status": "PASS", "aggregate_comparison": {}},
        )
        self.assertEqual(len(summary["EV04"]["metrics"]), 28)

    def test_ev04_aggregate_does_not_require_historical_run_metadata(self) -> None:
        canonical = runner.aggregate_baseline_metrics(
            "EV04",
        ...
        self.assertEqual(canonical, self.metrics)
