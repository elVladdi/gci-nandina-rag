...
VERSIONED_DEPENDENCY_PATHS = (
    ...
    "src/retrieval/bm25.py", "src/retrieval/text2trade_mnrl_v02.py", "src/bm25_index.py",
...
def dependency_bindings(root: Path) -> list[dict[str, Any]]:
    ...
    current = set(VERSIONED_DEPENDENCY_PATHS[:4])
    return [git_binding(root, path, candidate if path in current else "HEAD", "CANDIDATE_GIT_CANONICAL_BLOB" if path in current else "FROZEN_GIT_CANONICAL_BLOB") for path in VERSIONED_DEPENDENCY_PATHS]
...
def validate_manifest_payload(payload: Mapping[str, Any]) -> None:
    ...
    require(isinstance(environment, Mapping) and environment.get("python_version") == TESTED_ENVIRONMENT["python_version"] and environment.get("executable_sha256") == TESTED_ENVIRONMENT["executable_sha256"], "Execution manifest environment fingerprint is inconsistent")
...
def validate_authorization_baseline_ancestry(root: Path, baseline: str, revision: str = "HEAD") -> str:
    ...
    require(subprocess.run(["git", "merge-base", "--is-ancestor", baseline, current], cwd=root).returncode == 0, "Authorization baseline is not a proper ancestor")
    return current
...
def validate_dependency_bindings(root: Path, bindings: Any, revision: str = "HEAD") -> None:
    require(isinstance(bindings, list) and len(bindings) == len(VERSIONED_DEPENDENCY_PATHS) and {item.get("path") for item in bindings} == set(VERSIONED_DEPENDENCY_PATHS), "Dependency binding path set is not exact")
    for binding in bindings:
        expected = git_binding(root, binding["path"], revision, binding["classification"])
        require(binding == expected, f"Dependency Git binding drift: {binding['path']}")
...
def load_authorization_transition_from_git(root: Path, current_gate: Mapping[str, Any]) -> dict[str, Any]:
    ...
    validate_dependency_bindings(root, baseline_artifacts["unified_gate"]["candidate_source_bindings"], baseline)
    return {
        ...
        "record": record, "transition": transition, "artifacts": authorized_artifacts,
    }
...
STALE_SOURCE_TOKEN_ALLOWLIST = {
    ...
    "src/experiments/run_d1a_corrective_0b05c_v05.py": ("corrective_0b05c_v01 as legacy",),
    "src/experiments/run_0b05c_corrective_numerical_v05.py": (
...
def write_readiness(root: Path, environment: Mapping[str, Any], shadow: Mapping[str, Any]) -> str:
    ...
    payload = {
        ...
        "external_audit_findings": {f"F35C-0{index}": "CLOSED_BY_CODE_TEST_AND_SHADOW" for index in range(1, 7)},
