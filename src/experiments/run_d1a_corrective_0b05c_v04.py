"""Closed prospective D1a adapter for the isolated 0B-05C v0.4 roots."""
...
def preflight(root: Path = ROOT, *, revision: str = "HEAD") -> dict[str, Any]:
    ...
    require(spec["authorization"]["D1A_NUMERICAL_EXECUTION"] in {"NOT_AUTHORIZED", "AUTHORIZED"}, "D1a v0.4 authorization state is invalid")
    ...
    for relative in D1A_ROOTS:
        require(not (root / relative).exists(), f"D1a v0.4 future root exists: {relative}")
...
def execute_authorized(root: Path = ROOT, *, authorization_proof: Mapping[str, Any] | None = None) -> dict[str, Any]:
    if authorization_proof is None:
        from .run_0b05c_corrective_numerical_v04 import preflight_authorized

        authorization_proof = preflight_authorized(root)
    validate_unified_authorization_proof(authorization_proof)
    spec = read_json(root, SPEC_PATH)
    require(spec["authorization"]["D1A_NUMERICAL_EXECUTION"] == "AUTHORIZED", "D1a v0.4 numerical execution is NOT_AUTHORIZED")
