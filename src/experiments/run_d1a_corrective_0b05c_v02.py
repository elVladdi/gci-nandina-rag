"""Closed prospective D1a adapter for the 0B-05C v0.2 namespace."""
...
def preflight(root: Path = ROOT, *, revision: str = "HEAD") -> dict[str, Any]:
    ...
    require(spec["authorization"]["D1A_NUMERICAL_EXECUTION"] in {"NOT_AUTHORIZED", "AUTHORIZED"}, "D1a v0.2 authorization state is invalid")
    ...
    for relative in D1A_ROOTS:
        require(not (root / relative).exists(), f"D1a v0.2 future root exists: {relative}")
