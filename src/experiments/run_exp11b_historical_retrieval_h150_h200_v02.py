"""Fail-closed prospective EXP-11B H150/H200 retrieval runner v0.2.
...
DEFAULT_CONFIG = Path("src/configs/exp11b_historical_retrieval_h150_h200_execution_v0.2.json")
RUNNER_PATH = Path("src/experiments/run_exp11b_historical_retrieval_h150_h200_v02.py")
...
def _validate_authorization(
    ...
    for field, expected in required.items():
        if authorization.get(field) != expected:
            raise ContractViolation(f"OFFICIAL_EXECUTION_BLOCKED_AUTHORIZATION_BINDING_MISMATCH:{field}")
    return {**authorization, "execution_head": execution_head}
...
def _run_streaming_package(
    ...
    try:
        ...
        if before_first_scoring is not None:
            before_first_scoring()
        for spec in bank_specs:
            ...
            metrics, run = _evaluate_bank_streaming(
                identity, bank_rows, eval_rows, case_writer, candidate_writer, context
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate or prospectively execute the frozen EXP-11B H150/H200 historical BM25 package v0.2."
    )
