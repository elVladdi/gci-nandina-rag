...
def _candidate_columns(max_candidates: int) -> list[str]:
    ...
    for rank in range(1, max_candidates + 1):
        columns.extend([f"candidate_{rank}_code", f"candidate_{rank}_score", f"candidate_{rank}_doc_id", f"candidate_{rank}_text"])
    return columns
...
def _batched_embeddings(retriever: DenseText2TradeRetriever, queries: Sequence[str], batch_size: int) -> np.ndarray:
    if retriever.model is None:
        raise RuntimeError("Dense Text2Trade model was not loaded.")
...
def _summary_markdown(metrics: Mapping[str, Any]) -> str:
    ...
    hierarchical = metrics["hierarchical_metrics"]
    lines = [
        "# Evaluacion dense Text2Trade data_aduanas clase 87 v0.1",
        "",
        "## Objetivo",
        "",
        "Evaluar el artefacto dense Text2Trade local sobre el evalset `data_aduanas` de Clase = 87, sin reemplazar ni reinterpretar la Fase 5 historica de 600 casos.",
        "",
        "## Insumos",
        "",
        ...
        f"- Artefacto dense: `{metrics['input']['artifact_dir']}`.",
        f"- Modelo local: `{metrics['dense_config']['model_path']}`.",
        f"- Modo de recuperacion: {metrics['dense_config']['retrieval_mode']}.",
        f"- Profundidad de recuperacion: {metrics['dense_config']['retrieval_depth']}.",
        "",
        "## Metricas exactas",
        "",
        "| Metrica | Valor |",
    ...
    lines.extend(
        [
            ...
            "- Se uso el modelo SentenceTransformer local del artefacto Text2Trade.",
            "- `hnsw.index` no se reconstruyo; la recuperacion fue por fuerza bruta sobre `vectors.npy`.",
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    queries = [_clean(row.get(args.query_column)) for row in eval_rows]

    start = time.time()
    query_embeddings = _batched_embeddings(retriever, queries, args.batch_size)
    result_rows: list[dict[str, Any]] = []
    for position, (eval_row, query, query_embedding) in enumerate(zip(eval_rows, queries, query_embeddings), start=1):
        expected = _normalize_code(eval_row.get(args.label_column))
        hits = retriever.retrieve(query, top_k=depth, query_embedding=query_embedding)
    ...
    if global_metrics["zero_retrieval_cases"]:
        warnings.append("Some queries produced zero dense results.")

    metrics: dict[str, Any] = {
        "script": "src.experiments.evaluate_dense_text2trade_data_aduanas",
        "execution": {
        ...
        "input": {
            ...
            "artifact_abs_dir": str(artifact_dir),
        },
        "scope": {
            "source": "data_aduanas",
            "class": EXPECTED_SCOPE_CLASS,
            "evalset_role": "main evalset for updated Phase 5 after updated Phase 3",
        },
        "columns": {
        ...
        "dense_config": {
            **dense_summary,
            "artifact_dir": _rel(artifact_dir, root),
            "vectors_path": _rel(artifact_dir / "index" / "vectors.npy", root),
            "id_map_path": _rel(artifact_dir / "index" / "id_map.json", root),
            ...
            "docs_count": retriever.docs_count,
        ...
        "validations": {
            ...
            "text2trade_dense_model_loaded_locally": True,
            "brute_force_used": True,
        },
        "comparability_note": (
            "Historical Phase 5 v0.1 used evalset_v0.1.csv with 600 cases; this run uses data_aduanas "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate dense Text2Trade on data_aduanas Clase 87 evalset.")
...
def main() -> int:
    ...
    global_metrics = metrics["global_metrics"]
    hierarchical = metrics["hierarchical_metrics"]
    print("OK: evaluacion dense Text2Trade data_aduanas clase 87 completada")
