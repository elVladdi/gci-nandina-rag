...
from .prepare_0b05c_corrective_numerical_gate_v05 import (
    ...
    ROOT, ContractViolation, build_bundle, preauthorization_environment_preflight,
...
def preflight_authorized(root: Path = ROOT, *, interpreter: Path | str | None = None) -> dict[str, Any]:
    ...
    record = _read_json(root / AUTHORIZATION_RECORD)
    require(record.get("artifact_id") == "0b05c_numerical_authorization_record_v0.5" and record.get("external_audit") == "PASS", "v0.5 authorization record is invalid")
    ...
    return {
        ...
        "authorization_record_binding": {"path": AUTHORIZATION_RECORD.as_posix(), "sha256": hashlib.sha256((root / AUTHORIZATION_RECORD).read_bytes()).hexdigest()},
        "authorization_baseline_commit": record.get("authorization_baseline_commit"),
        "environment": environment, "bundle": {**bundle, **specs},
...
def validate_arm_runtime_integrity(
    ...
    require(isinstance(metrics, Mapping), f"{arm} metrics payload is malformed")
    names = evaluator.EV04_METRIC_ORDER if arm == "EV04" else EV03_AGGREGATE_ORDER
    evaluator.validate_metric_table(metrics, arm, names)
...
def _default_operations(root: Path, proof: Mapping[str, Any], *, interpreter: Path | str | None = None) -> dict[str, Callable[[], Mapping[str, Any]]]:
    ...
    def execute_d1a() -> Mapping[str, Any]:
        ...
        d1a_summary = common._d1a_summary_reference(root, _read_json(root / SPEC_PATHS["d1a"]), d1a_result)
    ...
    def manifest() -> Mapping[str, Any]:
        payload = {"artifact_id": "0b05c_corrective_execution_manifest_v0.5", "status": "PASS", "authorization_provenance": {"status": "PASS", "record": proof["authorization_record_binding"]}, "pipeline_steps": list(PIPELINE_STEPS), "future_roots": list(FUTURE_ROOTS), "results": {"control": control, "corrected": corrected, "d1a": d1a_result}}
    ...
    def ledger() -> Mapping[str, Any]:
        ...
        entries = validate_runtime_ledger(root, expected, produced, snapshot=relative_snapshot)
