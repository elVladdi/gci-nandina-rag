...
def _default_operations(root: Path, proof: Mapping[str, Any], *, interpreter: Path | str | None = None) -> dict[str, Callable[[], Mapping[str, Any]]]:
    ...
    def unified_preflight() -> Mapping[str, Any]:
        environment = {
            **proof["environment"]["interpreter"],
            "historical_vector_replay_required": proof["environment"]["historical_vector_replay_required"],
            "historical_vector_replay": proof["environment"]["historical_vector_replay"],
        }
        return _write_json_new(runtime_record, {"status": "PASS", "mode": "AUTHORIZED_PREFLIGHT_ONLY", "authorization": proof["authorization"], "environment": environment})
