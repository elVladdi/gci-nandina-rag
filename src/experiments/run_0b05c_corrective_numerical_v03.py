"""Fail-closed unified runner for a future authorized 0B-05C v0.3 run."""
...
def preflight_authorized(root: Path = ROOT) -> dict[str, Any]:
    ...
    require(gate.get("gate_status") == AUTHORIZED_GATE_STATUS, "Authorized v0.3 gate is not approved and integrated")
    require(gate.get("authorization_readiness") == AUTHORIZED_READINESS, "Authorized v0.3 gate readiness is invalid")
...
def _default_operations(root: Path, proof: Mapping[str, Any]) -> dict[str, Callable[[], Mapping[str, Any]]]:
    ...
    from . import evaluate_normative_bm25_corrective_0b05c_v03 as evaluator
    ...
    runtime_record = runtime_root / "runtime_authorization_record_v0.3.json"
    ...
    def compare_cases() -> Mapping[str, Any]:
        for arm, spec in specs.items():
            ...
            case_comparisons[arm] = _write_json_new(evaluation_root / f"{arm.lower()}_case_level_comparison_v0.3.json", {"status": "PASS", "rows": rows})
        return {"status": "PASS"}
    ...
    def compare_aggregates() -> Mapping[str, Any]:
        for arm, spec in specs.items():
            ...
            aggregate_comparisons[arm] = _write_json_new(evaluation_root / f"{arm.lower()}_aggregate_comparison_v0.3.json", {"status": "PASS", "metrics": evaluator.produce_aggregate_comparison(original, corrected[arm]["metrics"])})
        return {"status": "PASS"}
    ...
    def summary() -> Mapping[str, Any]:
        ...
        return _write_json_new(evaluation_root / "unified_sensitivity_summary_v0.3.json", payload)
    ...
    def manifest() -> Mapping[str, Any]:
        ...
        return _write_json_new(runtime_root / "execution_manifest_v0.3.json", payload)
    ...
    def ledger() -> Mapping[str, Any]:
        ...
        return _write_json_new(runtime_root / "exact_hash_ledger_v0.3.json", {"status": "PASS", "entries": entries, "mismatch_count": 0})
