...
def aggregate_baseline_metrics(
    ...
    """Return the frozen EV03 baseline or canonical case-derived EV04 baseline."""

    if arm == "EV04":
        expected = control_result.get("expected_metrics")
        require(isinstance(expected, Mapping), "EV04 aggregate baseline requires canonical expected control metrics")
        return dict(expected)
    metadata = root / spec["primary_original_control"]["run_metadata"]["path"]
    return json.loads(metadata.read_text(encoding="utf-8"))["metrics"]
...
def _default_operations(root: Path, proof: Mapping[str, Any]) -> dict[str, Callable[[], Mapping[str, Any]]]:
    ...
    from . import run_d1a_corrective_0b05c_v04 as d1a_runner
    ...
    def compare_aggregates() -> Mapping[str, Any]:
        for arm, spec in specs.items():
            original = aggregate_baseline_metrics(arm, spec, root, control[arm])
            compare = (
                evaluator.produce_ev04_aggregate_comparison_v04
                if arm == "EV04"
                else evaluator.produce_aggregate_comparison
            )
            aggregate_comparisons[arm] = _write_json_new(evaluation_root / f"{arm.lower()}_aggregate_comparison_v0.4.json", {"status": "PASS", "metrics": compare(original, corrected[arm]["metrics"])})
        return {"status": "PASS"}
