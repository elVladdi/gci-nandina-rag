...
Estructura para las tres comparaciones de H100 historico frente a los comparadores corregidos de Attempt06 en las cinco metricas primarias. Los valores absolutos `historical_observed_value` y `comparator_observed_value` son conceptos separados del estimando `paired_difference_historical_minus_comparator`. El `point_estimate`, `ci_lower` y `ci_upper` de G3-F03 pertenecen exclusivamente a esa diferencia pareada, con CI de 99%, y no representan incertidumbre de ninguno de los valores absolutos por brazo. No se autoriza calcular CI por brazo; Top-50 queda fuera de esta tabla por ser suplementario.
...
Cada tabla materializada por G5-F02 debera declarar unidad, poblacion, denominador, DAM cuando aplique, rol de incertidumbre y blob fuente. Los CI se copiaran solo desde G3-F03 y conservaran el estimando al que estan ligados. En G5-MAIN-01, el CI de 99% se liga al contraste pareado historico menos comparador y no a los valores absolutos por brazo; no existe CI por brazo autorizado por esta ficha salvo que una fuente aprobada lo materialice expresamente. `DESCRIPTIVE_NO_CI_AUTHORIZED` no se convertira en CI; `NOT_ESTIMABLE` no se convertira en cero. Los 10 x 1,056 registros EXP11B no son observaciones independientes.
