...
La fuente congelada es `data/Series - Descripciones.xlsx`, SHA-256 `db01d1fcdd41d1bd1ed8086fc6c19bcd56ba44b2534391aba7daa4c58f9f52d1`, con tamano de 7895186 bytes y orden de hojas `Hoja2`, `Hoja1`. La copia local se realiza mediante `shutil.copy2`, como bytes, y no se versiona en Git.
...
Las hojas historicas son `Hoja2` (indice 0) y `Hoja1` (indice 1). Las unicas hojas nuevas permitidas para la primera expansion son `NUEVA_01` y, opcionalmente, `NUEVA_02`. Deben agregarse en ese orden despues de las dos hojas historicas. Cualquier tercera hoja requiere un contrato prospectivo nuevo antes de procesarse.
...
El unico recorrido permitido sera: RAW EXCEL, PYTHON, NORMALIZED NEW SHEET, CURATION, ELIGIBLE NEW HISTORICAL POOL, AUDIT, HASHES, NEW_HISTORICAL_GATE. No se construiran H150, H200 ni CSV historicos ampliados en Excel.
...
Antes de la elegibilidad, se obtienen las DAM congeladas de DEV y EVAL. Toda nueva fila con `DECLARACION` coincidente queda `EXCLUDED_FIXED_DEV_EVAL_DAM`. Un `id_unico` ya presente en H100, DEV o EVAL se audita y no crea una segunda copia. Las coincidencias textuales exactas y near-duplicate 0.90, 0.95 y 0.98 se reportan como auditoria; no son exclusiones automaticas sin autorizacion metodologica.

H100 tiene 2950 filas, H150 apunta a 4425 y H200 a 5900. Despues de parsing, curacion y controles se requieren al menos 2950 filas nuevas elegibles netas. Si el total elegible es menor que 5900, el gate falla: no se reduce H200.
...
El manifiesto local queda bajo `outputs/audits/new_historical_gate_v0.1/`. La copia XLSX se archiva fuera del repositorio para no convertirla en un artefacto rastreable.
